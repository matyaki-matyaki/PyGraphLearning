...
def _calc_z(X: np.ndarray) -> np.ndarray:
    """
    Calculate a vector consisting only of the upper-right elements of the pair-wise distances matrix Z of the data matrix X
...
def _create_S_and_P(N: int, T: int) -> tuple[sparse.csr_array, sparse.csr_array]:
    """
    Creation of matrices S and P
    Create matrix S that satisfies \bm{S}\bm{w} = \bm{d}, and matrix P that satisfies \bm{P}\bm{w} = \bm{w} - \hat{\bm{w}}
    Where,
    - \bm{w}^{(t)}: A N(N-1)/2-dimensional vector composed of the upper-right elements of the adjacency matrix \bm{W}^{(t)} at time t
    - \bm{d}^{(t)}: A N-dimensional vector of degrees of the graph with adjacency matrix \bm{W}^{(t)} at time t
    - \bm{w} = [\bm{w}^{(1)} \bm{w}^{(2)} ... \bm{w}^{(T)}]
    - \bm{d} = [\bm{d}^{(1)} \bm{d}^{(2)} ... \bm{d}^{(T)}]
    - \hat{\bm{w}} = [\bm{w}^{(1)} \bm{w}^{(1)} ... \bm{w}^{(T-1)}] (vector \bm{w} shifted by one)
    ...
    for i in range(1, T):
        col_indices_S[N * (N-1) * i: N * (N-1) * (i+1)] = col_indices_S[N * (N - 1) * (i - 1) : N * (N - 1) * i] + C
    data_S = np.ones(N * (N - 1) * T, dtype = np.float64)
    S = sparse.csr_array((data_S, (row_indices_S, col_indices_S)), shape=(N * T, C * T))
    ...
    data_P[0::2] = -1
    P = sparse.csr_array((data_P, (row_indices_P, col_indices_P)), shape=(C * T, C * T))
...
def _PDS_tvgl(z: np.ndarray, S: sparse.csr_array, P: sparse.csr_array, alpha: float, beta: float, eta: float, gamma: float, maxit: int, epsilon: float) -> np.ndarray:
    ...
    -------
    np.ndarray
        The optimal solution (vector form of estimated adjacency matrix columns)
    """
    ...
    # def prox.
    prox_f = lambda x_f, gamma_: np.maximum(x_f - 2 * gamma * z, 0.)
    prox_g1 = lambda v_g1, gamma_: (v_g1 + np.sqrt(np.square(v_g1) + 4 * alpha * gamma_)) / 2
    prox_g2 = lambda v_g2, gamma_: np.sign(v_g2) * np.maximum(np.abs(v_g2) - eta * gamma_, 0.)
    prox_g1_ast = lambda v_g1_ast, gamma_: v_g1_ast - gamma_ * prox_g1(v_g1_ast / gamma_, 1 / gamma_)
    prox_g2_ast = lambda v_g2_ast, gamma_: v_g2_ast - gamma_ * prox_g2(v_g2_ast / gamma_, 1 / gamma_)
...
def tvgl(X: np.ndarray, alpha: float=1., beta: float=1e-2, eta: float=2.0, step: float=0.5, maxit: int=10000, epsilon: float=1e-5) -> np.ndarray:
    ...
    ----------
    X : np.ndarray
        Data matrix with K graph signals at each observation time; matrix shape is (T, N, K) (N: number of vertices)
    alpha : float, optional
        Hyperparameter (no need to adjust as it adjusts the scale of edge weights of the resulting graph), by default 1.
    beta : float, optional
