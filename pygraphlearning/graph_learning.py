...
from pygraphlearning.utils.static.static_graph_learning import sgl
from pygraphlearning.utils.time_varying.time_varying_graph_learning import tvgl
...
class GraphLearning(BaseEstimator, TransformerMixin):
    ...
    is_laplacian : bool, optional
        Whether to convert the adjacency matrix to the graph Laplacian and
        return it, by default False
    positivity_offset : float | None, optional
        How much offset to add to the graph Laplacian to make it
    ...
    eta : float | None, optional
        Hyperparameter (adjusting the variability of the time-varying graph),
        by default None
    step : float, optional
    ...
    def __init__(
        ...
        positivity_offset : float | None, optional
            ...
            a positive definite matrix., by default None
        alpha : float, optional
            Hyperparameter (adjusting the scale of edge weights of
            the resulting graph, so no adjustment needed), by default 1.
        beta : float, optional
            Hyperparameter (adjusting the sparcity of the graph),
            by default 1e-2
        eta : float | None, optional
            Hyperparameter (adjusting the variability of the time-varying
    ...
    def fit(self, X, y=None):
        ...
        ----------
        X : np.ndarray
            - For static graph learning: Data matrix consisting of K graph
                signals; the shape of the matrix is (N, K)
                (N: number of vertices)
            - For time-varying graph learning: Data matrix with K graph
                signals at each observation time; matrix shape is (T, N, K)
    ...
    def transform(self, X):
        """
        - For static graph learning: Estimate a weighted adjacency matrix or
            a graph laplacian matrix
        - For time-varying graph learing: Estimate weighted adjacency matrices
            or graph laplacians matrices
        ...
        np.ndarray
            ...
            - For time-varying graph learning: Series of estimated matrices
                (T, N, N)
        """
        if self.graph_type == 'static':
            W = sgl(
            ...
            if self.is_laplacian:
                ...
                if self.positivity_offset is not None:
                    L_array += \
                        self.positivity_offset * np.eye(L_array.shape[-1])
                return L_array
            else:
                return W_array
